...
std::set<int> read_logged_ports(const std::filesystem::path& path) {
    ...
    while (std::getline(stream, line)) {
        ...
        int port = 0;
        std::getline(parser, ordinal, '\t');
        std::getline(parser, method, '\t');
        std::getline(parser, http_version, '\t');
        parser >> port;
...
struct LoggedRequest {
    ...
    std::string response_content_range;
    std::string response_content_encoding;
    std::size_t body_bytes = 0;
};

std::string read_log_field(
...
std::vector<LoggedRequest> read_logged_requests(
    ...
    while (std::getline(stream, line)) {
        ...
        request.ordinal =
            static_cast<std::size_t>(
                std::stoull(read_log_field(parser)));
        request.method = read_log_field(parser);
        request.http_version = read_log_field(parser);
        request.client_port =
            std::stoi(read_log_field(parser));
        request.range_header = read_log_field(parser);
        request.accept_encoding = read_log_field(parser);
        request.response_status =
            std::stoi(read_log_field(parser));
        request.response_content_range =
            read_log_field(parser);
...
void write_legacy_resume_state(
    ...
    state.total_size = static_cast<std::int64_t>(total_size);
    state.vdl_offset = complete
    ...
    state.ranges.push_back({
        ...
        static_cast<std::uint8_t>(
            complete
                ? asyncdownload::core::RangeStatus::finished
                : asyncdownload::core::RangeStatus::paused)
    });

    asyncdownload::metadata::MetadataStore store(metadata_file);
...
TEST(HttpTransferCharacterizationTest, UsesHeadThenZeroZeroRangeAfterRejectedHead) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_probe_fallback");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    EXPECT_EQ(requests[0].response_status, 405);
    EXPECT_EQ(requests[1].method, "GET");
    EXPECT_EQ(requests[1].range_header, "bytes=0-0");
    EXPECT_EQ(requests[1].response_status, 206);
    EXPECT_TRUE(std::all_of(
        requests.begin(),
        requests.end(),
        [](const LoggedRequest& logged) {
            return logged.http_version == "HTTP/1.1";
        }));
    EXPECT_TRUE(files_equal(source_file, output_file));
...
TEST(HttpTransferCharacterizationTest, RejectsIgnoredPartialRangeWithoutRetry) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_ignored_partial");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    request.output_path = output_file;
    request.options.max_connections = 4;
    request.options.scheduler_window_bytes = 1024 * 1024;
...
TEST(HttpTransferContractTest, RejectsShiftedContentRangeBeforeBodyAdmission) {
    ...
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_shifted_range");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
...
TEST(HttpTransferCharacterizationTest, ReplaysPausedWritesWithoutDuplicateOutput) {
    ...
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_pause_replay");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto source_file = temp_root / "source.bin";
    const auto output_file = temp_root / "downloaded.bin";
    write_test_file(source_file, 16 * 1024 * 1024);
...
TEST(HttpTransferContractTest, FallsBackFromUnsolicitedHead206AndIgnoresPartialLength) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_head_206");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    ASSERT_GE(requests.size(), 3U);
    EXPECT_EQ(requests[0].method, "HEAD");
    EXPECT_EQ(requests[0].response_status, 206);
    EXPECT_EQ(requests[1].method, "GET");
    EXPECT_EQ(requests[1].range_header, "bytes=0-0");
    EXPECT_TRUE(files_equal(source_file, output_file));
...
TEST(HttpTransferContractTest, RejectsMalformedFallbackContentRange) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_malformed_probe_range");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    EXPECT_EQ(
        result.error,
        asyncdownload::make_error_code(
            asyncdownload::DownloadErrc::http_probe_failed));
    EXPECT_FALSE(std::filesystem::exists(output_file));
    EXPECT_FALSE(std::filesystem::exists(
        std::filesystem::path(output_file.string() + ".part")));
    EXPECT_FALSE(std::filesystem::exists(
...
TEST(HttpTransferContractTest, RejectsMismatchedFallbackTotal) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_probe_total_mismatch");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto server_command = quote_arg(L"python") + L" " +
        ...
        quote_arg(source_file.wstring()) +
        L" --port 0 --head-status 405" +
        L" --content-range-total-delta -1048576";
    ASSERT_TRUE(start_process(
        server,
        L"",
        server_command,
...
TEST(HttpTransferContractTest, AcceptRangesRequiresBytesToken) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_accept_ranges_token");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto server_command = quote_arg(L"python") + L" " +
        ...
        L" --port 0 --request-log " +
        quote_arg(request_log.wstring()) +
        L" --accept-ranges-value notbytes";
    ASSERT_TRUE(start_process(
        server,
        L"",
        server_command,
...
TEST(HttpTransferContractTest, FallsBackFromNon200HeadStatus) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_head_status");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto requests = read_logged_requests(request_log);
    ASSERT_GE(requests.size(), 3U);
    EXPECT_EQ(requests[0].response_status, 204);
    EXPECT_EQ(requests[1].method, "GET");
    EXPECT_EQ(requests[1].range_header, "bytes=0-0");
    EXPECT_TRUE(files_equal(source_file, output_file));
...
TEST(HttpTransferContractTest, FallsBackWhenHeadLengthIsMissing) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_head_length");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
...
TEST(HttpTransferContractTest, UsesOnlyFinalRedirectHeaderBlock) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_redirect_headers");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto server_command = quote_arg(L"python") + L" " +
        ...
        L" --port 0 --request-log " +
        quote_arg(request_log.wstring()) +
        L" --accept-ranges-value none" +
        L" --redirect-path /redirect";
    ASSERT_TRUE(start_process(
        server,
        L"",
        server_command,
    ...
    asyncdownload::DownloadRequest request{};
    request.url =
        std::string("http://127.0.0.1:") + port_line + "/redirect";
    request.output_path = output_file;
    request.options.max_connections = 4;
...
TEST(HttpTransferContractTest, RejectsZeroByteObjectByCurrentPolicy) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_http_zero_object");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
...
TEST(DownloadIntegrationTest, DowngradesNonRangeServerToSingleFullRequest) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_non_range_fresh");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
    ...
    const auto server_command = quote_arg(L"python") + L" " +
        quote_arg(script_path.wstring()) + L" " +
        quote_arg(source_file.wstring()) +
        L" --port 0 --chunk-size 65536 --request-log " +
        quote_arg(request_log.wstring()) +
        L" --disable-ranges";
    ASSERT_TRUE(start_process(
        server,
    ...
    request.progress_callback =
        [&max_active_requests](
            const asyncdownload::ProgressSnapshot& snapshot) {
            max_active_requests =
                std::max(max_active_requests, snapshot.active_requests);
        };
    ...
    EXPECT_LE(max_active_requests, 1U);
    EXPECT_TRUE(files_equal(source_file, output_file));
...
TEST(DownloadIntegrationTest, RestartsSparseResumeStateWhenRangeIsUnavailable) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_non_range_sparse");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);

    constexpr std::size_t block_size = 64 * 1024;
    constexpr std::size_t total_size = 4 * block_size;
    const auto source_file = temp_root / "source.bin";
    const auto output_file = temp_root / "downloaded.bin";
    const auto part_file =
        std::filesystem::path(output_file.string() + ".part");
    const auto metadata_file =
        std::filesystem::path(output_file.string() + ".config.json");
    const auto request_log = temp_root / "requests.log";
    write_test_file(source_file, total_size);
    ASSERT_TRUE(std::filesystem::copy_file(source_file, part_file, ec));
    ASSERT_FALSE(ec);
    std::vector<std::byte> corrupt_tail(
...
TEST(DownloadIntegrationTest, FinalizesAlreadyCompleteStateWithoutRangeSupport) {
#ifndef _WIN32
    GTEST_SKIP() << "This integration test currently uses Windows process control.";
#else
    const auto workspace_root = get_workspace_root();
    const auto temp_root =
        make_unique_temp_root("asyncdownload_non_range_complete");
    std::error_code ec;
    std::filesystem::create_directories(temp_root, ec);
    ASSERT_FALSE(ec);
